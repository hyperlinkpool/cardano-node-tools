...
public class MessageJP {
	...
	static {
		...
		messageFormats.put("M00138", "\n"
				...
				+ "5. ステークプールの情報登録（StakeのKeyを最初に登録する必要があり、プールの登録は500ADAがかかり、この後、更新の時からは送信の手数料のみかかります。)\n"
				+ "6. ステークプールの更新\n"
				+ "7. ステークプールの撤回（保証金として登録した500ADAを取り戻すことができます。）\n"
				+ "8. ステークプールの登録状況確認\n"
				+ "9. KESのKeyのRotation\n"
				+ "99. 戻る");
		...
		messageFormats.put("M00151", "現在の口座にADA残高が足りません。10ADA程度入金した後、再試行してください。");
		...
		messageFormats.put("M00175", "BlockProducerではありません。BlockProducerではない場合は、関連するKey（KES singing key、Cold signing key、Cold Counter key）があればKES Key Rotationを進めることができます。");
