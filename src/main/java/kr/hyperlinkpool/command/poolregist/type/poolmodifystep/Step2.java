...
public class Step2 implements PoolModifyResult, Ordered, JobProcess{
	...
	public void run() {
		...
		while(mainLoop) {
			...
			try {
				...
				poolModifyDomain.setPoolPledge(poolPledge);
			} catch (Exception e) {
			...
			try {
				...
				poolModifyDomain.setPoolCost(poolCost);
			} catch (Exception e) {
				MessagePrompter.promptMessage(MessageFactory.getInstance().getMessage("입력값이 잘못되었습니다. 다시 입력하세요.", "M00064"), true);
				continue;
			}
			...
			try {
				...
				poolModifyDomain.setPoolMargin(poolMargin);
			} catch (Exception e) {
				MessagePrompter.promptMessage(MessageFactory.getInstance().getMessage("입력값이 잘못되었습니다. 다시 입력하세요.", "M00064"), true);
				continue;
			}
			...
			try {
				...
				poolModifyDomain.setRelayNodeCount(relayNodeCount);
			} catch (Exception e) {
				MessagePrompter.promptMessage(MessageFactory.getInstance().getMessage("입력값이 잘못되었습니다. 다시 입력하세요.", "M00064"), true);
				continue;
			}
			...
			poolModifyDomain.setRelayNodesInfo(relayNodesInfo);
			
			MessagePrompter.promptMessage("*******************************************************************************************************************", true);
			MessagePrompter.promptMessage(MessageFactory.getInstance().getMessage("Pool 보증금 : %s", "M00075",String.valueOf(poolModifyDomain.getPoolPledge()) + NodeConstants.POSTFIX_LOVELACE), true);
			MessagePrompter.promptMessage(MessageFactory.getInstance().getMessage("Pool 고정 비용 : %s", "M00076",String.valueOf(poolModifyDomain.getPoolCost()) + NodeConstants.POSTFIX_LOVELACE), true);
			MessagePrompter.promptMessage(MessageFactory.getInstance().getMessage("Pool Margin : %s", "M00077",String.valueOf(poolModifyDomain.getPoolMargin())), true);
			for(int i=0;i<relayNodeCount;i++) {
