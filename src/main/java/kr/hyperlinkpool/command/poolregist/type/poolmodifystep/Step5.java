...
public class Step5 implements PoolModifyResult, Ordered, JobProcess{
	...
	public void run() {
		 ...
		 * Pool 등록
		 */
		long sumLovelace = poolModifyDomain.getSumLovelace();
		long receiverTotal = poolModifyDomain.getReceiverTotal();
		long poolDeposit = poolModifyDomain.getPoolDeposit();
		long txFee = poolModifyDomain.getTxFee();
		
		String txIn = poolModifyDomain.getTxIn();
		String txOut = poolModifyDomain.getTxOut();
					...
					if("Y".equalsIgnoreCase(inputValue)) {
						...
						while(loop) {
							...
							try {
								...
								poolModifyDomain.setTxFeeReCalc(true);
								poolModifyDomain.setTxFee(reCalcTxFee);
								return;
							} catch (Exception e) {
