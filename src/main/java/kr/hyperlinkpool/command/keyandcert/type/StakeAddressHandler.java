...
public class StakeAddressHandler extends AbstractCommandHandler{
	...
	public ProcessResultDomain <ProcessResponse> handleCommand(Object ... commands) {
		...
		String cardanoKeysStakeAddressOutputFilePath = cardanoKeysFolderString + NodeConstants.PATH_DELIMITER + NodeProperties.getString("cardano.keys.stake.addr");
		...
		File cardanoKeysStakeAddressOutputFile = new File(cardanoKeysStakeAddressOutputFilePath);
		if(cardanoKeysStakeAddressOutputFile.exists()) {
		...
		command = CommandExecutor.generateCommand(NodeCommandFormats.STAKE_ADDRESS, cardanoCliName, cardanoKeysStakeVkeyString, cardanoKeysStakeAddressOutputFilePath);
