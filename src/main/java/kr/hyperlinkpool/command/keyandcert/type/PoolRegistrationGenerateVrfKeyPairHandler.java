...
public class PoolRegistrationGenerateVrfKeyPairHandler extends AbstractCommandHandler{
	...
	public ProcessResultDomain <ProcessResponse> handleCommand(Object ... commands) {
		...
		String cardanoKeysVrfVkeyString = cardanoKeysFolderString + NodeConstants.PATH_DELIMITER + NodeProperties.getString("cardano.keys.vrf.vkey");
		String cardanoKeysVrfSkeyString = cardanoKeysFolderString + NodeConstants.PATH_DELIMITER + NodeProperties.getString("cardano.keys.vrf.skey");
		...
		File cardanoKeysVrfVkeyFile = new File(cardanoKeysVrfVkeyString);
		File cardanoKeysVrfSkeyFile = new File(cardanoKeysVrfSkeyString);
		
		if(cardanoKeysVrfVkeyFile.exists() || cardanoKeysVrfSkeyFile.exists()) {
		...
		String command = null;
		String cardanoCliCommand = "cardano-cli"; 
		command = CommandExecutor.generateCommand(NodeCommandFormats.POOL_REGISTRATION_GENERATE_VRF_KEY_PAIR, cardanoCliCommand, cardanoKeysVrfVkeyString, cardanoKeysVrfSkeyString);
