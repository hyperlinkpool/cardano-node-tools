...
public class StakeKeyPairHandler extends AbstractCommandHandler{
	...
	public ProcessResultDomain <ProcessResponse> handleCommand(Object ... commands) {
		...
		String cardanoKeysStakeSkeyString = cardanoKeysFolderString + NodeConstants.PATH_DELIMITER + NodeProperties.getString("cardano.keys.stake.skey");
		...
		File cardanoKeysStakeSkeyFile = new File(cardanoKeysStakeSkeyString);
		
		if(cardanoKeysStakeVkeyFile.exists() || cardanoKeysStakeSkeyFile.exists()) {
		...
		command = CommandExecutor.generateCommand(NodeCommandFormats.STAKE_KEY_PAIR, cardanoCliName, cardanoKeysStakeVkeyString, cardanoKeysStakeSkeyString);
